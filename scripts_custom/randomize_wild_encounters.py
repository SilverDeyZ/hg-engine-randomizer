#!/usr/bin/env python3
"""Randomize wild encounter species in armips/data/encounters.s."""
...
ENCOUNTER_RE = re.compile(
    r"^(?P<prefix>\s*encounter\s+)(?P<species>SPECIES_[A-Z0-9_]+)(?P<suffix>\s*,.*)$"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Randomize wild encounter species in armips/data/encounters.s using "
...
def randomize_encounters(text: str, allowed_species: list[str], rng: random.Random) -> tuple[str, int]:
    changed = 0
    ...
    for line in text.splitlines(keepends=True):
        ...
        if encounter_match:
            original = encounter_match.group("species")
            if original != "SPECIES_NONE":
                replacement = rng.choice(allowed_species)
                body = f"{encounter_match.group('prefix')}{replacement}{encounter_match.group('suffix')}"
                changed += 1
            updated_lines.append(body + newline)
            continue
...
def main() -> int:
    print("========================================")
    print(" Wild Encounter Randomizer")
    print("========================================")
    ...
    updated, changed = randomize_encounters(original, allowed_species, rng)
    ...
    if updated != original:
        ...
        backup_status = str(DEFAULT_BACKUP)
