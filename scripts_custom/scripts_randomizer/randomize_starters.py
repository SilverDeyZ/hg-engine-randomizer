#!/usr/bin/env python3
"""Randomize starter species in armips/data/starters.s."""
...
DEFAULT_STARTERS = Path("armips/data/starters.s")
DEFAULT_SPECIES_HEADER = Path("include/constants/species.h")
DEFAULT_BACKUP = Path("bak/starters.s.bak")
DEFAULT_MIN = 1
DEFAULT_MAX = 1075
EXCLUDED_IDS = set(range(494, 545))

DEFINE_RE = re.compile(r"^\s*#define\s+(?P<name>[A-Z0-9_]+)\s+(?P<expr>.+?)\s*(?://.*)?$")
STARTER_RE = re.compile(r"^(?P<prefix>\s*\.word\s+)(?P<species>SPECIES_[A-Z0-9_]+)(?P<suffix>\s*(?://.*)?)$")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Randomize starter species in armips/data/starters.s using "
    ...
    parser.add_argument(
        "--starters",
        type=Path,
        default=DEFAULT_STARTERS,
        help=f"Starter file to edit (default: {DEFAULT_STARTERS}).",
    )
    parser.add_argument(
        "--species-header",
        type=Path,
...
def build_allowed_species(species_by_id: dict[int, str], min_species: int, max_species: int) -> list[str]:
    ...
    if not allowed:
        raise ValueError("No valid species found in the requested range.")
    return allowed


def randomize_starters(text: str, allowed_species: list[str], rng: random.Random) -> tuple[str, int]:
    changed = 0
    ...
    for line in text.splitlines(keepends=True):
        ...
        starter_match = STARTER_RE.match(body)
        if starter_match:
            original = starter_match.group("species")
            if original != "SPECIES_NONE":
                replacement = rng.choice(allowed_species)
                body = f"{starter_match.group('prefix')}{replacement}{starter_match.group('suffix')}"
                changed += 1
...
def main() -> int:
    ...
    original = args.starters.read_text(encoding="utf-8")
    updated, changed = randomize_starters(original, allowed_species, rng)

    if updated != original:
        print(f"\nCreating backup: {DEFAULT_BACKUP}")
        write_backup(original, DEFAULT_BACKUP)
        args.starters.write_text(updated, encoding="utf-8")

    print()
    print("Done.")
    print(f"  Updated entries : {changed}")
    print(f"  Starters file   : {args.starters}")
    return 0
